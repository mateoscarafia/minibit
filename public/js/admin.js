...
const showResults = (res_inner) => {
  ...
  const resultContainer = document.getElementById(
    "exam-results-items-container-id"
  );
...
const hideResume = (res) => {
  const resumeContainer = document.getElementById("resumen-container-id");
  resumeContainer.style.top = "-1000px";
};

const showResume = (res) => {
  try {
    const resumeContainer = document.getElementById("resumen-container-id");
    resumeContainer.style.top = "100px";

    const exam = content_question
      .filter((ctn) => ctn.id == res.content_id)
      .pop();
    const bad_answers_array = res.bad_answers.split("/");

    const filtered = {};

    bad_answers_array.forEach((ans) => {
    ...
    exam.questions.forEach((ques, index) => {
      ...
      if (filtered[ques.id]) {
        resultHTML =
          resultHTML +
          `<div class="resumen-results-item">${index + 1}. ${
            ques.question
          }</div>
...
const searchResults = () => {
  ...
  const filtered = results.filter((res) => {
    if (email && content)
      return res.email.includes(email) && res.content_id == Number(content);
...
const showContent = () => {
  ...
  content_techs.forEach((tech) => {
    contentContainer.innerHTML += `<div class="content-item">
            ...
            <a style="width:10px" target="_blank" href="${
              window.location.protocol
            }//${window.location.host}/content-files/${tech.filename}">pdf</a>
           <span style="width:10px" onclick="statusContent(${
             tech.content_id
           }, ${tech.enable})">${tech.enable ? "✅" : "⛔️"}</span>
            <span style="width:10px" onclick="deleteContent(${
              tech.content_id
            })">x</span>
          </div>`;
  });
  ...
  fetch("/enable-content", {
    ...
    .then(() => {
      ...
      content_techs = content_techs.map((res) => {
        if (res.content_id == id)
          return { ...res, enable: status ? null : "1" };
        return res;
      });
      showContent();
    })
...
const showUsers = (user_inner) => {
  ...
  const amountContents = content_question.filter(
    (cnt) => cnt.questions.length
  ).length;
  ...
  user_inner.forEach((user) => {
    ...
    userContainer.innerHTML += `<div class="user-item user-score-${userScore}">
            ...
            <span class="smaller-user-span">${
              userResults.length
            }/${amountContents}</span>
            <span style="font-weight: bold" class="smaller-user-span">${
              isNaN(prom) ? 0 : prom
            }%</span>
...
const displayQuestionForm = (questions) => {
  ...
  [...Array(50).keys()].forEach((item, index) => {
    if (questions[index]) {
      document.getElementById(`question_${index}_id`).value =
        questions[index].question;
      document.getElementById(`answer_${index}_a_id`).value =
        questions[index].answer_a;
      document.getElementById(`answer_${index}_b_id`).value =
        questions[index].answer_b;
      document.getElementById(`answer_${index}_c_id`).value =
        questions[index].answer_c;
      document.getElementById(`answer_${index}_d_id`).value =
...
const displayContainers = (container) => {
  ...
  if (container == "usuarios")
    document.getElementById("users-container-id").style.display = "block";
  if (container == "resultados")
    document.getElementById("exam-results-id").style.display = "block";
  if (container == "contenido")
    document.getElementById("content-container-id").style.display = "block";
  if (container == "examenes")
    document.getElementById("tests-container-id").style.display = "block";
};
    ...
    fetch("/create-content", {
      ...
      .then(async (response) => {
        if (response.ok) {
          ...
          content_techs.unshift({
            name: result.name,
            filename: result.filename,
            content_id: result.id,
          });
          content_question.unshift({
            name: result.name,
            id: result.id,
            questions: [],
          });
